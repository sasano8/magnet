...
class AccessToken(BaseModel):
    ...
    scopes: Dict[str, str] = {"me": "Read information about the current user.", "items": "Read items."}
    ...
    def encode_access_token(self, data: dict, expires_minutes: int = 30) -> bytes:
        ...
        encoded_jwt = jwt.encode(to_encode, self.secret_key.get_secret_value(), algorithm=self.algorithm)
        return encoded_jwt

    def decode_access_token(self, token: str) -> Mapping:
        payload = jwt.decode(token, self.secret_key.get_secret_value(), algorithms=[self.algorithm])
        return payload
