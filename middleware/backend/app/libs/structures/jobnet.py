...
class Dag(BaseModel):
    ...
    def get_ready(self):
        ...
        nodes = self.get_all().filter(lambda x: x == "not_running").to_dict(lambda x: x.name)

        for node in nodes.values():
            depends = Linq(node.depends).filter(lambda x: not x == "success").map(lambda x: x.name).to_list()
