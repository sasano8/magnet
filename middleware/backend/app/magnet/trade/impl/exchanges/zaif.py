...
class TradeResponse(BaseModel):
  ...
  class Config:
    schema_extra = {
        "example": {"received": 12.7972899, "remains": 0, "order_id": 0, "funds": {"jpy": 112637.6587618, "btc": 0, "xem": 3745.2, "mona": 0, "ZAIF": 1016.1602}}
    }
...
class ZaifPublicAPI():
    ...
    async def request_public(self, url: str, params: dict = {}):
        ...
        signature = hmac.new(bytearray(self.api_secret.encode("utf-8")), digestmod=hashlib.sha512)
...
class ZaifTradingAPI(ZaifPublicAPI):
        ...
        async def get_id_info(self):
            ...
            """
            url = f"https://api.zaif.jp/tapi"
            params = dict(method="get_id_info", nonce=datetime.datetime.utcnow().timestamp())
            res = await self.request(url=url, params=params)
            return res.text
        ...
        async def get_active_orders(self,
            ...
            """
            url = f"https://api.zaif.jp/tapi"
            params = dict(method="active_orders", nonce=datetime.datetime.utcnow().timestamp(),
                          currency_pair=currency_pair,
                          is_token=is_token,
        ...
        async def post_cancel_order(self, order_id: int, currency_pair: str = None, is_token: bool = False):
            ...
            """
            url = f"https://api.zaif.jp/tapi"
            params = dict(method="cancel_order", nonce=datetime.datetime.utcnow().timestamp(), order_id=order_id,
                          currency_pair=currency_pair,
                          is_token=is_token)
            res = await self.request(url=url, params=params)
            return res.text
        ...
        @decorators.Decode
        async def post_withdraw(self, currency: str, address: str, amount: float, message: str = None, opt_fee: float = None):
            """
            ...
            """
            url = f"https://api.zaif.jp/tapi"
            params = dict(method="withdraw", nonce=datetime.datetime.utcnow().timestamp(),
                          currency=currency,
                          address=address,
                          amount=amount,
                          message=message,
        ...
        async def get_deposit_history(self,
            ...
            """
            url = f"https://api.zaif.jp/tapi"
            params = dict(method="deposit_history", nonce=datetime.datetime.utcnow().timestamp(),
                          currency=currency,
                          count=count,
                          from_id=from_id,
                          end_id=end_id,
...
class ZaifMarginTradingAPI(ZaifTradingAPI):
    ...
    async def get_positions(self,
        ...
        params = dict(method="get_positions", nonce=datetime.datetime.utcnow().timestamp(), type="margin",
                      group_id=group_id,
                      count=count,
                      from_id=from_id,
                      end_id=end_id,
    ...
    @decorators.Decode
    async def get_position_history(self,
                                   leverage_id: int,
                                   group_id: int = None
                            ):
        """
        ...
        """
        url = f"https://api.zaif.jp/tlapi"
        params = dict(method="position_history", nonce=datetime.datetime.utcnow().timestamp(), type="margin",
    ...
    async def get_active_positions(self,
        ...
        """
        url = f"https://api.zaif.jp/tlapi"
        params = dict(method="active_positions", nonce=datetime.datetime.utcnow().timestamp(), type="margin",
                      group_id=group_id,
                      currency_pair=currency_pair
                      )
        res = await self.request(url=url, params=params)
    ...
    async def post_create_position(self,
        ...
        """
        url = f"https://api.zaif.jp/tlapi"
        params = dict(method="create_position", nonce=datetime.datetime.utcnow().timestamp(), type="margin",
                      currency_pair=currency_pair,
                      action=action,
                      amount=amount,
                      price=price,
    ...
    async def post_change_position(self,
        ...
        """
        url = f"https://api.zaif.jp/tlapi"
        params = dict(method="change_position", nonce=datetime.datetime.utcnow().timestamp(), type="margin",
                      currency_pair=currency_pair,
                      leverage_id=leverage_id,
                      price=price,
                      group_id=group_id,
    ...
    @decorators.Decode
    async def post_cancel_position(self,
                                   leverage_id: float,
                                   group_id: int = None,
                                   ):
        """
        ...
        """
        url = f"https://api.zaif.jp/tlapi"
        params = dict(method="get___", nonce=datetime.datetime.utcnow().timestamp(), type="margin",
                      leverage_id=leverage_id,
                      group_id=group_id
                      )
        res = await self.request(url=url, params=params)
