...
from .errors import TradeException
...
class OrderLogic(BaseModel):
    ...
    allocation_rate: decimal.Decimal = Field(0.7, gt=0, lt=1, description="財布から何％を割り当てるか指定する")
    min_unit: decimal.Decimal = Field(0.01, gt=0, description="最低取引単位　ビットコインは0.01")
    ask_limit_rate: decimal.Decimal = Field(None, gt=1, description="ask時に何％の損益で利確するか指定する")
    ask_loss_rate: decimal.Decimal = Field(None, lt=1, description="ask時に何％の減益でロスカットするか指定する")
    bid_limit_rate: decimal.Decimal = Field(None, gt=1, description="bid時に何％の損益で利確するか指定する")
    bid_loss_rate: decimal.Decimal = Field(None, lt=1, description="bid時に何％の減益でロスカットするか指定する")
...
class TradeProfile(BaseModel):
    ...
    job_type: Literal["production", "virtual", "backtest"] = "backtest"
    bet_strategy: Literal["flat", "martingale", "grand_martingale", "parley", "dalambert", "pyramid"] = "flat"
    ...
    def calc_amount(self, price):
        ...
        amount = amount.quantize(self.order_logic.min_unit, rounding=decimal.ROUND_FLOOR)
        if self.order_logic.min_unit > amount:
            raise TradeException("最低取引単位以下の注文を出すことはできません。もっと財布にお金を入れてください。")
        return amount
