import pytest
from libs.linq import Linq, BaseModel, convert_to_queryable
import itertools
import operator
import numpy
...
def test_linq_compare():
    ...
    query = Linq(source[0])
    assert all([a == b for a, b in itertools.zip_longest(query.__root__, [0, None, True, obj0])])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[0]), [True, True, True, True])])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[1]), [False, True, True, True])])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[2]), [False, True, True, True])])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[3]), [True, True, True, True])])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[4]), [True, True, True, False])])
    ...
    query = Linq(source[0])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[0]), [True])])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[1]), [True])])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[2]), [False])])
    ...
    query = Linq(source[0])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[0]), [True, True])])
    assert all([a == b for a, b in itertools.zip_longest(query.compare(source[1]), [True, False])])
